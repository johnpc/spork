Feature: Play a slideshow quiz (name the artist for each album)
  ...
  # timer, and types real artists. Asserts on REAL rendered slide data — the
  # first slide's album prompt, then that the deck ADVANCES to the next prompt
  # after a correct answer and the score increments — not just navigation.
  ...
  Scenario: A guest names artists and the deck advances slide by slide
    ...
    Then the slide prompt shows "Thriller (1982)"
    When the player starts the quiz
    And the player types the answer "Michael Jackson"
    Then the score shows "1"
    And the slide prompt shows "Rumours (1977)"
    When the player types the answer "Pink Floyd"
  ...
  Scenario: A wrong artist does not advance the deck or score
    ...
    And the slide prompt shows "Thriller (1982)"
