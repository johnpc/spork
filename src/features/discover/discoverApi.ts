...
export async function fetchShelves(): Promise<Shelf[]> {
  ...
  // Categories are a small reference set — list + shape client-side. unwrap so a
  // GraphQL/network failure throws (→ retry) instead of silently rendering empty.
  const categories = unwrap(await dataClient.models.Category.list({ limit: 200, authMode }));
